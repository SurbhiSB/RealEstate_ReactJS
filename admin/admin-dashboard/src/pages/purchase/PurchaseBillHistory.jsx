...
export default function purchaseBillHistory() {
...
useEffect(() => {
  const fetchBills = async () => {
    try {
      const res = await axios.get("http://localhost:3000/api/purchase-bills");
      if (res.data.success) {
  ...
  {bills.length === 0 ? (
    ...
    bills.map((bill, index) => (
      <tr key={bill._id}>
        ...
        <td className="px-4 py-2">{bill.billDate?.slice(0, 10)}</td>
        <td className="px-4 py-2">{bill.billNumber}</td>
        <td className="px-4 py-2">{bill.vendorName}</td>
        <td className="px-4 py-2">{bill.projectName}</td>
        <td className="px-4 py-2">{bill.dueDate?.slice(0, 10)}</td>
