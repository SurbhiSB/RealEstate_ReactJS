...
useEffect(() => {
  ...
  setPayments([]); // clear previous
    ...
    if (res.data.success) {
      setPayments([...res.data.data]); // clone data to force update
    } else {
      setPayments([]);
    }
  } catch (err) {
        ...
        <div className="p-4">
          ...
          <div className="bg-white p-4 rounded-lg shadow mb-4 flex flex-wrap gap-4 items-end">
            ...
            <button
              ...
              disabled={loading}
              className={`bg-gray-800 text-white px-4 py-2 rounded-lg mt-1 hover:bg-gray-900 ${loading ? "opacity-50 cursor-not-allowed" : ""
                }`}
            >
                ...
                {payments.length === 0 ? (
                  ...
                  payments.map((pay, index) => (
                    <tr key={`${pay._id}-${index}`}>
                      <td className="px-4 py-2">{pay.person || "-"}</td>
          ...
          <div className="flex justify-between items-center text-sm mt-4">
            <div>
              Showing {payments.length === 0 ? 0 : 1} to {payments.length} of {payments.length} entries
            </div>
